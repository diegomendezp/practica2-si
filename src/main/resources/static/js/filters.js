const filterByNoDemandados = servicios => servicios
...
const filterByMasIngresos = (servicios) => {
  let items = servicios.filter(e => e.solicitudes.length > 0);
  items = items.map((e) => {
    e.solicitudesTotales = e.solicitudes.filter(solicitud => solicitud.estado == 'Completada').length;
	  e.precio_acumulado = getSumOfImportes(e.solicitudes);
	  return e;
  });
  return items.length > 1
    ? items.sort(
      (e, e2) => e2.precio_acumulado - e.precio_acumulado,
    )
    : items;
};

const getSumOfImportes = solicitudes => solicitudes.reduce((a, b) => {
  if (a.importe) return a.importe + b.importe;
  return a + b.importe;
}, 0);

const filterProfesionalesByMasDemandados = (servicios) => {
  const items = servicios.filter(e => e.solicitudes.length > 0);
  return items.length > 1
    ? items.sort((e, e2) => {
      const index = e2.solicitudes.length - e.solicitudes.length;
      if (index === 0) {
...
const compareStrings = (s1, s2) => (s1.toLowerCase() < s2.toLowerCase()
  ? -1
